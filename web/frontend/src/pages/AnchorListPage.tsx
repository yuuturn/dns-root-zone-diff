...
import { fetchAnchorDiffs, formatDetectedAt, type DiffListResponse } from "../api.ts";
import { SummaryBadges } from "../components/badges.tsx";

const PER_PAGE = 20;

export function AnchorListPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    setError(null);
    fetchAnchorDiffs(page, PER_PAGE)
      .then((resp) => {
  ...
  if (error) {
    return (
      <Banner
        ...
        variant="error"
        title="Failed to load root anchor diffs"
        description={error}
      />
    );
  }
  ...
  if (data.total === 0) {
    return (
      <Empty
        ...
        title="No root anchor changes recorded yet"
        description="Root anchor changes will appear here once they are detected."
      />
    );
  }
        ...
        <Table>
          <Table.Header>
            <Table.Row>
              ...
              <Table.Head>Trust anchor id</Table.Head>
          ...
          <Table.Body>
            {data.diffs.map((d) => (
              <Table.Row
                ...
                onClick={() => navigate(`/anchors/diffs/${d.id}`)}
              >
